...
if (!global.paused) {
	...
	if !myPlayer.hit && !myPlayer.stun && myPlayer.charge_att { //attack button pressed 
		...
		hitbox_timer += 1
			...
			else if player2 = "Wizard" { //Widen hitbox
				image_xscale *= 1.3
				image_yscale *= 1.3
			...
			else if player2 = "Jouster" { //lunge forward
			...
			if collision_circle(x,y,1,obj_player1,true,false) {
				myPlayer.charge_att = false
				activated = false
				obj_player1.push_amount = 10
				myPlayer.move_dir = obj_player2.move_dir
	...
	if myPlayer.attacking { //change sprite 
		...
		else if player2 = "Knight" or player2 = "Jouster" {
